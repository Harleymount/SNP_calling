...
#vcf1<-'/Users/Harley/Desktop/high_quality_SNPS_clinical.csv'
#vcf2<-'/Users/Harley/Desktop/high_quality_SNPS_hottub.csv'
...
#------------------Adding part to give unique SNPs list ----------------------------------
unique_snps_sample_1<-sample1[!(sample1$unique_SNP_ID %in% sample2$unique_SNP_ID),]
unique_snps_sample_2<-sample2[!(sample2$unique_SNP_ID %in% sample1$unique_SNP_ID),] 
unique_snps_sample_1<-cbind(unique_snps_sample_1, sample=sample_name_1)
unique_snps_sample_2<-cbind(unique_snps_sample_2, sample=sample_name_2)
...
merged<-rbind(unique_snps_sample_1,unique_snps_sample_2)
merged$X<-NULL
col_names<-c('contig','position','ref','alternate','qual','info','format','ploidy_info','contig', 'mapper','unique_SNP_ID', 'sample_name')
colnames(merged)<-col_names 
write.csv(merged, 'unique_SNPs.csv')
