...
dir=args[1]
reference=args[2]
dir='/Users/Harley/Desktop/SNP_analysis/clinical'
reference='/Users/Harley/Desktop/SNP_analysis/clinical/sequence.fasta'
library(seqinr)   
...
if (exists('bowtie2_filtered_df')){ #added this if exists check to see if the dataframe is actually present before I start doing stuff with it 
    ...
    for (i in 1:length(bowtie2_filtered_df[,1])){
        #contig_info<-unlist(strsplit(as.character(bowtie2_filtered_df$V1[i]), '_')) #this is necessary for references which were de novo assembled, but if they werent just need
        #this code
        #contig_identifier<-paste0(contig_info[1],'_',  contig_info[2])
        contig_identifier<-as.character(bowtie2_filtered_df$V1[i]) #HAD TO CHANGE THIS TO GET CLINICAL TO WORK  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for (i in 1:length(bowtie2_filtered_df[,1])){
        ...
        if (length(query_table_clusters[,1]) >= 3 ){ # has to be 3 because it will always incdue itself, so if it includes itself and two others there are three SNPs within 10 kb and need to be eliminated
            SNP_clusters<-c(SNP_clusters, paste0(as.character(bowtie2_filtered_df$V1[i]),"_",bowtie2_filtered_df$V2[i]))} # had to change this and add to the item added to SNP clusters because it ws just adding the chromosome name, I updated so it adds chromosome name and also position pasyted together liek AEXXX_0912029#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    }
    ...
    #need to write a new code like that above to check if the contig_position is in SNP clusters, not just contig as code above checks, can probably just do past0 
    bowtie2_filtered_df<-bowtie2_filtered_df[!(paste0(bowtie2_filtered_df$V1,'_',bowtie2_filtered_df$V2) %in% SNP_clusters),]#========================================================I CHANGED THIS SEE ABOVE =============================================================================================================================================================
    bowtie<-c()
    for (i in 1:length(bowtie2_filtered_df[,1])){
...
if (exists('novoalign_filtered_df')){
    ...
    for (i in 1:length(novoalign_filtered_df[,1])){
        ...
        #contig_identifier<-paste0(contig_info[1],'_',  contig_info[2])
        contig_identifier<-as.character(novoalign_filtered_df$V1[i])
        contig_list<-c(contig_list, contig_identifier)
    }
    #add the identifier column onto the dataframe so that we can use these names to filter out later 
    ...
    for (i in 1:length(novoalign_filtered_df[,1])){
        ...
        query_table_clusters<-query_table_clusters[as.integer(query_table_clusters$V2) < upper_bound,]
        if (length(query_table_clusters[,1]) >= 3 ){ # has to be 3 because it will always incdue itself, so if it includes itself and two others there are three SNPs within 10 kb and need to be eliminated
            SNP_clusters<-c(SNP_clusters, paste0(as.character(novoalign_filtered_df$V1[i]),"_",novoalign_filtered_df$V2[i]))}
    }
    #with SNP clusters identified these should now be filtered out of the dataframe!
...
if (exists('GSNAP_filtered_df')){
    ...
    for (i in 1:length(GSNAP_filtered_df[,1])){
        ...
        #contig_identifier<-paste0(contig_info[1],'_',  contig_info[2])
        contig_identifier<-as.character(GSNAP_filtered_df$V1[i])
        contig_list<-c(contig_list, contig_identifier)
    }
    ...
    for (i in 1:length(GSNAP_filtered_df[,1])){
        ...
        query_table_clusters<-query_table_clusters[as.integer(query_table_clusters$V2) < upper_bound,]
        if (length(query_table_clusters[,1]) >= 3 ){ # has to be 3 because it will always incdue itself, so if it includes itself and two others there are three SNPs within 10 kb and need to be eliminated
            SNP_clusters<-c(SNP_clusters, paste0(as.character(GSNAP_filtered_df$V1[i]),"_",GSNAP_filtered_df$V2[i]))}
    }
    #with SNP clusters identified these should now be filtered out of the dataframe!
