...
#-----------------map reads to reference using SOAP2 
...
vcffilter -f 'QUAL > 30 & DP > 20 & AB > 0.2 & QUAL / AO > 10 & SAF > 0 & SAR > 0 & RPR > 1 & RPL > 1' $sample_name-bowtie.vcf > $sample_name-filtered-bowtie2.vcf;
#SNP quality has to be over 30 (QUAL), coverage over 20 (DP), and allele balance at heterozygous sites should exceed 0.2 (AB) for the alterate allele
...
#and at least two reads balanced on either side of the mutation
...
vcffilter -f 'QUAL > 30 & DP > 20 & AB > 0.2 & QUAL / AO > 10 & SAF > 0 & SAR > 0 & RPR > 1 & RPL > 1' $sample_name-novoalign.vcf > $sample_name-filtered-novoalign.vcf;

#------------------SOAP SNP calling
freebayes --fasta-reference $reference $sample_name-final_GSNAP.bam --ploidy 1 > $sample_name-GSNAP.vcf;
vcffilter -f 'QUAL > 30 & DP > 20 & AB > 0.2 & QUAL / AO > 10 & SAF > 0 & SAR > 0 & RPR > 1 & RPL > 1' $sample_name-GSNAP.vcf > $sample_name-filtered-GSNAP.vcf;
